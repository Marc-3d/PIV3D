...
=#
function zeroReplace( coords::NTuple{N,Int64}, radius::Int64,
                      arrays::Array{T,N}... ) where {T,N}
    return zeros( T, length(arrays) )
end

function medianReplace( coords::NTuple{N,Int64}, radius::Int64,
                        arrays::Array{T,N}... ) where {T,N}
...
function meanReplace( coords::NTuple{N,Int64}, radius::Int64,
                      arrays::Array{T,N}... ) where {T,N}
    mincoords = max.( 1, coords .- radius );
    maxcoords = min.( size(arrays[1]), coords .+ radius );
...
"""
    ...
    Implemented replacement functions are.
        ...
        PIV3D.meanReplace ----> replaces outlier vectors by the mean vector
"""
function magnitudeFilter( threshold, u::Array{T,N}, v::Array{T,N} ) where {T<:Real,N}
...
function spaceAveraging( avg_radius::Int64, u::Array{T,2}, v::Array{T,2};
                         th=0.1 ) where {T<:Real}
...
function spaceAveraging( avg_radius::Int64, U::Array{T,3}, V::Array{T,3}, W::Array{T,3} ) where {T<:Real}
...
function spaceTimeAveraging( avg_rs::Int64, avg_rt::Int64, u::Array{T,3}, v::Array{T,3}; th=0.0 )  where {T<:Real}
...
""" space-time averaging """ # fancier circular mask in notes_16Oct20
function spaceTimeAveraging( avg_rs::Int64, avg_rt::Int64, u::Array{T,4}, v::Array{T,4}, w::Array{T,4}; th=0.0 )  where {T<:Real}
...
function similarityAveraging3D( avg_radius::Int64, U::Array{T,3}, V::Array{T,3}, W::Array{T,3};
                                st=0.0 ) where {T<:Real}
                ...
                for z in zmin:zmax
                    for x in cmin:cmax
                        for y in rmin:rmax
                            ...
                            mag2 = sqrt( U[y,x,z]*U[y,x,z] + V[y,x,z]*V[y,x,z] + W[y,x,z]*W[y,x,z] )
                            nu2  = U[y,x,z]/mag2
                            nv2  = V[y,x,z]/mag2
                            nw2  = W[y,x,z]/mag2
...
""" Spatial averaging """
function similarityMap( avg_radius::Int64, U::Array{T,3}, V::Array{T,3}, W::Array{T,3};
                        st=0.0 ) where {T<:Real}
...
function velocityMap( U::Array{T,3}, V::Array{T,3}, W::Array{T,3} ) where {T<:Real}
...
# 2D
function sink( radius::Int64, pos::II )
    return [ [ (pos[1] - y )/sqrt( (pos[1]-y)^2 + (pos[2]-x)^2 ),
               (pos[2] - x )/sqrt( (pos[1]-y)^2 + (pos[2]-x)^2 ) ] for y in pos[1]-radius:pos[1]+radius,
                                                                       x in pos[2]-radius:pos[2]+radius ]
end

# 3D
function sink( radius::Int64, pos::III )
    return [ [ (pos[1] - y )/sqrt( (pos[1]-y)^2 + (pos[2]-x)^2 + (pos[3]-z)^2 ),
               (pos[2] - x )/sqrt( (pos[1]-y)^2 + (pos[2]-x)^2 + (pos[3]-z)^2 ),
               (pos[3] - z )/sqrt( (pos[1]-y)^2 + (pos[2]-x)^2 + (pos[3]-z)^2 ) ] for y in pos[1]-radius:pos[1]+radius,
                                                                                      x in pos[2]-radius:pos[2]+radius,
                                                                                      z in pos[3]-radius:pos[3]+radius ]
end
...
function divergenceMap( r::Int64, U::Array{T,3}, V::Array{T,3}, W::Array{T,3}  ) where {T<:Real}
    ...
    cmatrix = zeros( Float64, size(NU) .+ 2*r )
...
function PIVtrajectories( U::Array{P,4}, V::Array{P,4}, W::Array{P,4},
    ...
    numT = T1 - T0;
    TrajectoriesV = zeros( Float64, numT, numpoints )
    TrajectoriesL = zeros( Float64, numT, numpoints )
    TrajectoriesD = zeros( Float64, numT, numpoints )
    ...
    for pidx in 1:numpoints
        ...
        for t in 2:numT
            ...
            c1 = round( Int64, TrajectoriesV[t,pidx] / facV )
            c2 = round( Int64, TrajectoriesL[t,pidx] / facL )
            c3 = round( Int64, TrajectoriesD[t,pidx] / facD )
