...
"""
    Saves gif traversing the volume along the Z axis. 
"""
function volumeToGif( volume::Array{T, 3} ; filename="/tmp.gif"::String ) where {T<:Real}
    anim = @animate for i=1:size(volume, 3)
         Plots.plot( Images.Gray.( volume[ : , :, i ] ), showaxis=false );
    end
    Plots.gif( anim, string( pwd(), filename ), fps=2 )
end