...
function loadVTKVolume( filename::String; typ=nothing, path=pwd() )
    ...
    lines = readlines( fn );
    ( lines[2] !== "PIV3D.jl volume" ) && (error("The vtk volume was not stored with PIV3D, and cannot open it" ))

    w,  h,  d = parse.( Int64 , split( lines[5], " " )[2:4] );
...
function VTKVectorFieldSize( filename::String; path=pwd() )
    ...
    lines = readlines(fn);
    return parse.( Int64, split(lines[5]," ")[2:4] )
end
...
function loadVTKVectorField( filename::String; typ=nothing, path=pwd() )
    ...
    lines = readlines(fn);
    ( lines[2] !== "PIV3D.jl vector field" ) && (error("The vtk vector field was not stored with PIV3D, and cannot open it" ))
...
function volumeToVTK( filename::String, volume::Array{T,3}; path="", mode="w" ) where {T<:Real}
...
# volume at different positions
function volumeToVTK( filename::String, volume::Array{T,3}, pos::Tuple{Int64,Int64,Int64}; path="", mode="w" ) where {T<:Real}
    ...
    io = open( fn, mode )
        ...
        println( io, "DATASET STRUCTURED_POINTS"  );
        println( io, "DIMENSIONS $w $h $d"  );
        println( io, "ORIGIN $(pos[1]) $(pos[2]) $(pos[3])"  );
        println( io, "SPACING 1 1 1"  );
        println( io, "POINT_DATA $(length(volume))"  );
        println( io, "SCALARS intensities $data_type"  );
        println( io, "LOOKUP_TABLE default" );
